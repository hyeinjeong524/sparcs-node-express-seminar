...
const AccountPage = () => {
  ...
  const getAccountInformation = () => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/getInfo', { credential: SAPIKEY, password:Password });
  ...
  const performTransaction = ( amount: number | '' ) => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/transaction', { credential: SAPIKEY, password:Password, amount: amount });
      setNTransaction(0);
      if (!data.success) {
  ...
  return (
    <div className={"account"}>
      ...
      <div className={"account-token-input"}>
        Enter ID: <input type={"text"} value={SAPIKEY} onChange={e => setSAPIKEY(e.target.value)}/>
